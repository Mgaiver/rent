...
def init_firestore():
    ...
    try:
        ...
        st.warning(f"Não foi possível conectar ao Firestore. A persistência de dados está desativada. Verifique suas credenciais nos segredos. Erro: {e}")
        return None
...
if not st.session_state.clientes:
    ...
    for cliente, operacoes in list(st.session_state.clientes.items()):
        ...
        if dados_para_df:
            ...
            cols_metricas[0].metric(label="Lucro/Prejuízo Total Líquido", value=f"R$ {lucro_total_liquido:,.2f}", delta=f"{rentabilidade_total:,.2f}% sobre o total investido")
